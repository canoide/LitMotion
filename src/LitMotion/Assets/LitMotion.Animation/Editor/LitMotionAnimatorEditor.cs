...
namespace LitMotion.Animation.Editor
{
    ...
    public sealed class LitMotionAnimatorEditor : UnityEditor.Editor
    {
        ...
        VisualElement CreateAnimationEntryGUI(SerializedProperty entryProp, int entryIndex)
        {
            var box = new Box();
            ...
            header.Add(new Label($"#{entryIndex} "));
            header.Add(idField);
            ...
            var removeBtn = new Button(() => {
                animationsProp.DeleteArrayElementAtIndex(entryIndex);
                serializedObject.ApplyModifiedProperties();
                RefreshAnimationsList();
            })
            {
            ...
            foldout.viewDataKey = $"LitMotionAnim_{target.GetInstanceID()}_Entry_{entryIndex}";
